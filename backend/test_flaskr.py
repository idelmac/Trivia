...
class TriviaTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.database_path = 'postgresql://{}:{}@{}/{}'.format('postgres','12345678','localhost:5432', self.database_name)
        ...
        #self.new_question = {"question": "The best club", "answer": "Real Madrid", "difficulty": 1, "category": 6}
    ...
    def test_200_create_new_question(self):
        question = {"question": "The best club", "answer": "Real Madrid", "difficulty": 1, "category": 6}
    ...
    def test_400_if_question_bad_request(self):
        question = {"question": "The best club", "answer": "Real Madrid", "category": 6}
        res = self.client().post("/questions", json=question)
        data = json.loads(res.data)
    ...
    def test_404_if_question_category_does_not_exist(self):
        question = {"question": "The best club", "answer": "Real Madrid", "difficulty": 1, "category": 9999}
        res = self.client().post("/questions", json=question)
        data = json.loads(res.data)
    ...
    def test_409_if_question_is_duplicated(self):
        question = {"question": "What is the largest lake in Africa?", "answer": "Lake Victoria", "difficulty": 1, "category": 3}
        res = self.client().post("/questions", json=question)
        data = json.loads(res.data)
    ...
    def test_404_if_category_questions_does_not_exist(self):
        res = self.client().post("/categories/9999/questions")
        data = json.loads(res.data)
    ...
    # def test_200_get_category_questions_without_results(self):
    #     res = self.client().post("/categories/9999/questions")
    #     data = json.loads(res.data)
    ...
    def test_200_get_quizz(self):
        ...
        res = self.client().post("/quizzes", json={"previous_questions": previous_questions, "quiz_category": category})
        data = json.loads(res.data)
        
        question = Question.query.filter(~Question.id.in_(previous_questions), Question.category == category['id']).order_by(Question.id).one_or_none()
